...
def match_master_address(addr: str, master_by_pref: Dict[str, pd.DataFrame]) -> Optional[Tuple[Dict[str, Optional[str]], Optional[int]]]:
    addr_norm = normalize_address(addr)
    if not addr_norm:  # 空ならマッチしない
        return None
    pref = find_prefecture(addr_norm, list(master_by_pref.keys()))  # まず都道府県を特定
    if not pref:  # 都道府県が取れないと突合不可
        return None
    df_pref = master_by_pref.get(pref)  # 都道府県ごとの候補行
    if df_pref is None or df_pref.empty:  # 候補なし
        return None
    ...
    # 長い市区町村+町域を優先するため、長さ順にソートしたDataFrameで探索
    ...
    # 1. 完全一致（都道府県+市区町村+町域の正規化文字列がaddr_normの先頭に一致）
    for _, row in df_pref_sorted.iterrows():
        ...
        if full_norm and addr_norm.startswith(full_norm):
            ...
            idx_used = row.name
            return result, idx_used

    # 2. 市区町村＋町域の両方が含まれているか（部分一致）
    for _, row in df_pref_sorted.iterrows():
        city = safe_strip(row["市区町村名(漢字)"])
        town = safe_strip(row["町域名(漢字)"])
    ...
    for col in addr_cols:
        ...
        for addr in unique_addrs:
            ...
            if addr_norm not in cache:
                matched = match_master_address(addr_norm, master_by_pref)
                # マッチした行のインデックスを記録
                if matched is not None:
                    vals_dict, idx_val = matched
                    cache[addr_norm] = (vals_dict, idx_val)
                    if idx_val is not None:
                        used_master_idx.add(idx_val)
                else:
                    cache[addr_norm] = (None, None)
        # 行に結果を展開
        records = []
